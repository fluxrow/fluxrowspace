...
export async function POST(req: Request, { params }: { params: { workspaceId: string } }) {
    try {
        ...
        const currentCount = await db.workspaceMember.count({ where: { workspace_id: params.workspaceId } });
        const canAdd = await checkLimit(params.workspaceId, 'teamMembers', currentCount);
